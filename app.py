...
BOT_USERNAME = os.getenv('BOT_USERNAME', 'LoIzDealsbot')
...
def _event_label(event) -> str:
    ...
    mapping = {
        "create_deal": "нажал «Создать сделку»",
        "my_deals": "открыл «Мои сделки»",
        "requisites": "открыл «Реквизиты»",
        "details": "открыл «Подробнее»",
        "referrals": "открыл «Рефералы»",
        "language": "открыл «Язык»",
        "problem_report": "открыл поддержку",
        "check_join_request": "проверил заявку на доступ",
        "accept_terms": "принял условия сервиса",
        "back_to_start": "вернулся в главное меню",
        "withdraw": "открыл вывод средств",
        "req_more": "открыл дополнительные валюты",
    }
    if data in mapping:
        return mapping[data]
    if data.startswith("set_lang_"):
        lang = data.removeprefix("set_lang_")
        names = {
            "ru": "русский", "en": "английский", "zh": "китайский",
            "uk": "украинский", "uz": "узбекский", "ar": "арабский",
        }
        return f"выбрал язык: {names.get(lang, lang)}"
    if data.startswith("edit_"):
        cur = data.removeprefix("edit_").upper()
        return f"изменил реквизиты: {cur}"
    if data.startswith("pay_deal_"):
        return "перешёл к оплате сделки"
    if data.startswith("bind_req_"):
        return "привязал реквизиты к сделке"
    if data.startswith("deal_") or data.startswith("confirm_"):
        return "выполнил действие со сделкой"
    if data.startswith("adm_"):
        return "выполнил действие в админ-панели"
    if data.startswith("join_"):
        return "выполнил действие с заявкой на вход"
...
async def activity_log_middleware(handler, event: types.Message, data: dict):
    ...
    if text.startswith("/"):
        ...
        command_labels = {
            "/start": "запустил бота / открыл главное меню",
            "/admin": "открыл админ-панель",
            "/badwork": "включил режим без логирования",
            "/add": "запустил пополнение баланса",
            "/on": "включил бота",
            "/off": "выключил бота",
            "/work": "включил рабочий режим",
        }
        action = command_labels.get(command, f"выполнил команду {command}")
...
async def main():
    # Telegram не может одновременно использовать webhook и long polling.
    # Удаляем старый webhook перед запуском polling.
    try:
        await bot.delete_webhook(drop_pending_updates=True)
        print_console_log("Webhook удалён, запускаем polling.", "SUCCESS")
    except Exception as e:
        logging.error(f"Не удалось удалить webhook: {e}. Продолжаем запуск.")
